...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns, isComplexTypeElement) {
  var self = this;
  var schema = this.definitions.schemas[xmlns];
  ...
  if (Array.isArray(obj)) {
    ...
    for (name in obj) {
        ...
        if (self.definitions.schemas) {
          if (schema) {
            ...
            if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
              ...
              dcNameSpace = obj[self.options.attributesKey].xsi_type.namespace;
              nonSubNameSpace = dcNameSpace + ':';
              ...
              var hasComplexTypeElement = false;
              if (typeof obj[name] === 'object')
              {
                hasComplexTypeElement = true;
                // console.log('hasComplexTypeElement: ' + name);
              }
              value = self.objectToXML(child, name, namespace, xmlns, false, null, null, ancXmlns, hasComplexTypeElement);
            }
          } else {
      ...
      if (!Array.isArray(child)) {
        var _ns = nonSubNameSpace || ns;
        var _ns2 = ':' + _ns.replace(':', '');
        if (_ns2 === ':') _ns2 = '';
        var element = ['<', _ns, name, attr, xmlnsAttrib, (child === null ? ' xsi:nil="true"' : ''), '>'];
        if (isComplexTypeElement && this.definitions.complexTypeSchema)
        {
          element =  ['<', _ns, name, attr, xmlnsAttrib, (child === null ? ' xsi:nil="true"' : ''), ' xmlns', _ns2, '="', this.definitions.complexTypeSchema.$targetNamespace, '" xmlns="http://www.w3.org/2001/XMLSchema-instance"', '>'];
          // console.log(element.join(''));
        }
        
        parts.push(element.join(''));
      }
